...
# Event 1 is the kernel's poll timer.
ok_if(1, $event_id == 2);

# Kernel should have two events due.  A nonexistent session should
# have zero.

ok_if(2, $poe_kernel->_data_ev_get_count_from($poe_kernel) == 2);
ok_if(3, $poe_kernel->_data_ev_get_count_to($poe_kernel) == 2);
ok_if(4, $poe_kernel->_data_ev_get_count_from("nothing") == 0);
ok_if(5, $poe_kernel->_data_ev_get_count_to("nothing") == 0);

# Poll timer, session, and from/to for the event we enqueued.
ok_if(6, $poe_kernel->_data_ses_refcount($poe_kernel) == 4);
...
# Kernel should have one event due.
ok_if(7, $poe_kernel->_data_ev_get_count_from($poe_kernel) == 1);
ok_if(8, $poe_kernel->_data_ev_get_count_to($poe_kernel) == 1);

# Poll timer, and session.  From/to have been dispatched.
ok_if(9, $poe_kernel->_data_ses_refcount($poe_kernel) == 2);
...
ok_if(10, $poe_kernel->_data_ses_refcount($poe_kernel) == 10);
ok_if(11, $poe_kernel->_data_ev_get_count_from($poe_kernel) == 5);
ok_if(12, $poe_kernel->_data_ev_get_count_to($poe_kernel) == 5);
...
ok_if(15, $poe_kernel->_data_ev_get_count_from($poe_kernel) == 4);
ok_if(16, $poe_kernel->_data_ev_get_count_to($poe_kernel) == 4);
ok_if(17, $poe_kernel->_data_ses_refcount($poe_kernel) == 8);

# Remove an alarm by name, except that this is for a nonexistent
# session.
$poe_kernel->_data_ev_clear_alarm_by_name(BOGUS_SESSION, "timer");
ok_if(18, $poe_kernel->_data_ev_get_count_from($poe_kernel) == 4);
ok_if(19, $poe_kernel->_data_ev_get_count_to($poe_kernel) == 4);
ok_if(20, $poe_kernel->_data_ses_refcount($poe_kernel) == 8);
...
ok_if(24, $poe_kernel->_data_ev_get_count_from($poe_kernel) == 2);
ok_if(25, $poe_kernel->_data_ev_get_count_to($poe_kernel) == 2);
ok_if(26, $poe_kernel->_data_ses_refcount($poe_kernel) == 4);
...
ok_if(30, $poe_kernel->_data_ev_get_count_from($poe_kernel) == 1);
ok_if(31, $poe_kernel->_data_ev_get_count_to($poe_kernel) == 1);
ok_if(32, $poe_kernel->_data_ses_refcount($poe_kernel) == 2);
