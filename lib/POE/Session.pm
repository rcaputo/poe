...
The _default event is a re-thrown version of an event that had no
handler.  POE::Session determines when an event cannot be dispatched
to a handler, and it rewraps and rethrows the event within a _default
event.

POE::Kernel has very little knowledge about the events a session can
handle.  Therefore it's perfectly acceptable to post events to a
session that cannot handle them.  In most cases, the unhandled event
will be silently dropped.  _default allows a session to catch these
events rather than allow them to be lost.
...
When this occurs, POE::Session tries to redispatch them
